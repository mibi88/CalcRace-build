...
function clear(){ for(i=0;i!=window.w*window.h*4;i++){ data[i] = 255; } }
function get_mouse_x(){ return window.mx; }
function get_mouse_y(){ return window.my; }
function getheight(){ return window.h; }
function getkey(key){ return kmap[key]; }
function getwidth(){ return window.w; }
...
function init_canvas(width,height,canvasname){ window.canvas = document.getElementById(UTF8ToString(canvasname)); window.canvas.width = width; window.canvas.height = height; window.ctx = window.canvas.getContext("2d", antialias = 0); window.ctx.imageSmoothingEnabled = false; window.image = window.ctx.createImageData(width, height); window.image.imageSmoothingEnabled = false; window.data = image.data; window.w = window.canvas.width; window.h = window.canvas.height; }
function init_click(){ window.click = 0; window.canvas.addEventListener("mousedown", function(event) { window.click = 1; window.mx = Number(event.offsetX/(window.canvas.offsetWidth/window.w)); window.my = Number(event.offsetY/(window.canvas.offsetHeight/window.h)); }); window.canvas.addEventListener("mouseup", function(event) { window.click = 0; window.mx = Number(event.offsetX/(window.canvas.offsetWidth/window.w)); window.my = Number(event.offsetY/(window.canvas.offsetHeight/window.h)); }); }
function init_getkey(){ kmap = {}; document.onkeyup = function(e) { kmap[e.keyCode] = false; }; document.onkeydown = function(e) { kmap[e.keyCode] = true; }; }
function init_mouse(){ window.mx = 0; window.my = 0; window.canvas.addEventListener("mousemove", (event) => { window.mx = Number(event.offsetX/(window.canvas.offsetWidth/window.w)); window.my = Number(event.offsetY/(window.canvas.offsetHeight/window.h)); }); }
function init_touch(){ window.click = 0; window.canvas.addEventListener("touchstart", function(event) { canvas_rect = window.canvas.getBoundingClientRect(); window.mx = Number((event.changedTouches.item(0).pageX - canvas_rect.left)/(window.canvas.offsetWidth/window.w)); window.my = Number((event.changedTouches.item(0).pageY - canvas_rect.top)/(window.canvas.offsetHeight/window.h)); window.click = 1; }); window.canvas.addEventListener("touchend", function(event) { window.click = 0; }); }
function init_touch_move(){ window.mx = 0; window.my = 0; window.canvas.addEventListener("mousemove", (event) => { window.mx = Number(event.offsetX/(window.canvas.offsetWidth/window.w)); window.my = Number(event.offsetY/(window.canvas.offsetHeight/window.h)); }); }
function is_clicked(){ return window.click; }
function ms_time(){ return Date.now(); }
function put_pixel(x,y,r,g,b,a){ if(x >= 0 && x < window.w && y >= 0 && y < window.h){ var index = (y * window.w + x) * 4; data[index + 0] = r; data[index + 1] = g; data[index + 2] = b; data[index + 3] = a; } }
